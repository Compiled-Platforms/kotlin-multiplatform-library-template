...
- ✅ **Kotest Assertions** - Expressive assertions with better error messages
...
The template includes `kotest-assertions-core` for more expressive assertions, **without** requiring the full Kotest framework. You still use kotlin.test's `@Test` annotation and test structure.

### Why Kotest Assertions?
...
```kotlin
// kotlin.test: "expected:<5> but was:<6>"
assertEquals(5, result)

// Kotest: Shows context, actual vs expected, and suggestions
result shouldBe 5
// Expected: 5
// Actual: 6
...
fun testStrings() {
    ...
    text shouldEndWith "!"
    text shouldMatch "Hello.*"
    text.shouldNotBeEmpty()
    text.length shouldBe 13
}
```
...
You can mix kotlin.test and Kotest assertions in the same test:
...
fun testMixedStyles() {
    ...
    // Use whichever is clearer for each case!
}
```

### When to Use Kotest Assertions

✅ **Use Kotest when:**
- You want more readable tests
- You need better error messages
- Testing complex collections or strings
- You prefer infix syntax
...
**Both work great!** Choose based on your preference and team style.

### Available in Convention Plugin

Kotest assertions are automatically available in all libraries via the convention plugin:
...
See `KotestAssertionsExampleTest.kt` for comprehensive examples.
